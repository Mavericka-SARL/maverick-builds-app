...
var auditPolicy = grpcutil.AuditPolicy{
	"GenerateMigration": auditv1.EventCategory_EVENT_CATEGORY_MODEL_CHANGE,
	"ApplyMigration":    auditv1.EventCategory_EVENT_CATEGORY_MODEL_CHANGE,
	"RollbackMigration": auditv1.EventCategory_EVENT_CATEGORY_MODEL_CHANGE,
}

var deliberatelyUnaudited = []string{"PreviewMigration", "ListMigrations"}

func main() {
	log := logger.New("schema-migration")
	...
	store := schemamigration.NewStore(pool)
	generator := schemamigration.NewGenerator(pool)
	...
	schemamigrationv1.RegisterSchemaMigrationServiceServer(srv, schemamigration.NewServer(log, store, generator))
